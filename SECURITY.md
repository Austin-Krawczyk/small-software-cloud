...
| App config/secrets are separate from platform config | Per-app env vars live in their own table, are owner-only over the API, and are injected at process start; reserved launch keys (`PORT`, `PATH`, `SCLOUD_DATA_DIR`, …) can't be overridden (`lib/runner.ts`) |
| Each app has its own durable storage | A per-app folder is bind-mounted at `/data` (env `SCLOUD_DATA_DIR`); it lives outside the build dir so redeploys don't wipe it, is isolated per project, and is removed when the project is deleted (`lib/deploy.ts`) |
| Managed databases are per-project and isolated | An attached SQLite database is a file inside that one app's durable storage (`DATABASE_URL` / `SCLOUD_DATABASE_PATH`); no shared database server, so there's no cross-project database surface to reach |
