...
import { invitesOf, membersOf, shareWithEmail } from "@/lib/projects";
...
// Share the project with an email address.
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const status = shareWithEmail(id, email);
...
export async function DELETE(req: NextRequest, { params }: Params) {
  const { id } = await params;
  const { actor, error } = await requireProject(id, { ownerOnly: true });
  if (error) return error;
